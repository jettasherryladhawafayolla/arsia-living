...
if (!isset($_SESSION['id_user'])) {
    echo json_encode(["success" => false, "message" => "silahkan login terlebih 
    dahulu!"]);
    exit;
}
...
$query_id = "SELEC MAX(id_jual) AS last_id FROM tb_jual";
...
//Insert ke tb_jual
$query_jual = "INSERT INTO tb_jual (id_user, tgl_jual, total, diskon)
               VALUES ('$new_id', '$id_user', '$tgl_jual', '$total_bayar',
               '$diskon')";
if (!mysqli_query($koneksi,$query_jual)) {
    die(json_encode(["success" => false, "message" => "Gagal Insert ke tb_jual;
...
if (!empty($values)) {
    ...
    if (!mysqli_query($koneksi, $query_jualdtl)) {
        die(json_encode(["success" => false, "message" => "Gagal insert ke
        tb_jualdtl: " . mysqli_error($koneksi)]));
    }
}
...
foreach ($pesanan_data as $pesanan) {
    ...
    $query_update_stok = "UPDATE tb_produk SET stok = stok - $qty WHERE id_produk
    = '$id_produk'";

    if (!mysqli_query($koneksi, $query_update_stok)) {
        die(json_encode(["success" => false, "message" => "Gagal hapus tb_pesanan:
        " . mysqli_error($koneksi)]));
    }
}
